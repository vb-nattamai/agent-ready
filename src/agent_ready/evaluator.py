...
# ── AgentReady-generated file filter ─────────────────────────────────────────
# These files are written by AgentReady itself. They must never be treated as
# source files when the eval builds ground truth or source context.

AGENTREADY_GENERATED_FILES = {
    ...
    "AGENTIC_EVAL.md",
    "cost_report.json",
    # GitHub-specific generated files (checked by path below)
    ".github/copilot-instructions.md",
    ".github/CODEOWNERS",
    ".github/dependabot.yml",
}

# Directory prefixes whose entire contents are generated by AgentReady
_AGENTREADY_GENERATED_DIRS = {"memory", "skills", "hooks", "tools", ".agent-ready"}

# .github files generated by AgentReady (not CI source files)
_AGENTREADY_GITHUB_FILES = {"copilot-instructions.md", "CODEOWNERS", "dependabot.yml"}


def _is_agentready_generated(file_path: str) -> bool:
    """Return True if this file was generated by AgentReady and should be
    excluded from source context during eval.

    .github/workflows/ CI files are NOT excluded — they are real source.
    Only the specific .github files AgentReady generates are excluded.
    """
    p = Path(file_path)

    # Exact filename match at any depth
    if p.name in AGENTREADY_GENERATED_FILES:
        return True

    # Directory prefix: any file inside memory/, skills/, hooks/, etc.
    parts = p.parts
    for generated_dir in _AGENTREADY_GENERATED_DIRS:
        if generated_dir in parts:
            return True

    # .github generated files (not workflows/ci.yml etc.)
    if len(parts) >= 2 and parts[0] == ".github":
        if p.name in _AGENTREADY_GITHUB_FILES:
            return True
...
def _build_context_system(target: Path) -> str | None:
    ...
    The generated files (AGENTS.md, CLAUDE.md, memory/schema.md, skills/, etc.)
    are provided as AGENT INSTRUCTIONS, not as source files. A scaffolding notice
    is prepended so the model does not confuse these generated files with actual
    source repository structure.
    """
    # Detect which AgentReady-generated dirs and files actually exist in this target
    generated_present: list[str] = []
    for item in ["memory/", "skills/", "hooks/", "tools/"]:
        if (target / item.rstrip("/")).exists():
            generated_present.append(item)
    for fname in [".github/copilot-instructions.md", ".github/CODEOWNERS"]:
        if (target / fname).exists():
            generated_present.append(fname)
    ...
    # Scaffolding notice: tell the model which directories/files are generated
    # scaffolding so it does not report them as real source repository structure.
    if generated_present:
        parts.append("## ⚠️ AgentReady Scaffolding Notice")
        parts.append(
            "The following files and directories were generated by AgentReady to help "
            "you work with this repository. They are NOT part of the original source code "
            "and must NOT be described as source directories or source files when answering "
            "questions about repository structure, entry points, or project layout:"
        )
        for item in generated_present:
            parts.append(f"  - {item}")
        parts.append(
            "The actual source files are: app.py (or equivalent entry point), tests/, "
            ".github/workflows/ (CI), and the config files (pyproject.toml, Makefile, etc.)."
        )
        parts.append("")
...
def run_eval(
    ...
    source_files: dict[str, str] = {
        ...
        if not _is_agentready_generated(k)
    }
