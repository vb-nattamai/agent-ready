...
- provider-aware LLM client, cost attribution per provider, multi-provider pricing

### Changed

- docs: update eval results to 12/19 (63%), 26% hallucination — post all fixes
- chore: update hello_world_source eval and cost report post-rebase
- chore: sync hello_world_output with post-10B regeneration — 12/19 (63%), 26% hallucination
...
- eliminate safety hallucinations and fix domain concept extraction in generation prompts

### Changed

- chore: regenerate hello_world_source after 10A — ruff and pyproject.toml now in context files
...
- detect ruff from pyproject.toml and prioritise pyproject deps over requirements.txt in verified_facts

### Changed

- chore: regenerate hello_world_source context files with all extractor fixes applied
...
- exclude AgentReady scaffolding from eval source context to eliminate scaffolding pollution
...
- add hello_world source repo and regenerate context files with corrected eval design

### Fixed

- eval ground truth extractor — Makefile commands, pyproject priority, ruff detection
...
- verified_facts manifest + --eval-only mode + cost tracking fix
...
- add --eval-only mode to evaluate any repo context files without transformation

### Changed

- docs: regenerate hello_world example output with eval report
- docs: add openapi.yaml and tools/ to Generated Artifacts table
- docs: add --eval, --eval-only, and --fail-level to CLI reference table
...
- add hello_world example output and cost/time documentation

### Fixed

- harden skills/hooks pipeline — error handling, safe static access, conditional references

### Changed

- docs: add Skills and Hooks section and missing README additions
- docs: update all docs to reflect v2.8.0 new artifacts
...
- add skills/, hooks/, and .cursorrules artifact generation

### Changed

- docs: add spec.md and plan.md for new artifact types
...
- resolve ruff lint violations in test files

### Changed

- docs: update README to reflect v2.7.0 LLM-powered pipeline and eval framework
